...
class PLSDPipeline(DiffusionPipeline):
    ...
    def guidance_callback(
        ...
        # iG2 = avg_pool(operator.illum)
        # iG1, iG2, iH1, iH2 = self.pyramid.get(operator.illum)

        # meas_inv = operator.transpose(measurement, operator.illum, is_latent=True, adaptive_beta=use_adpative_beta)
        meas_inv = operator.transpose(measurement, is_latent=True, adaptive_beta=use_adpative_beta)
        meas_pred = operator.forward(origi_pred, is_latent=True, adaptive_beta=False)

        invG1, invG2, invH1, invH2 = self.pyramid.get(meas_inv)
        # factor = (1.0-0.5) * ((operator.illum - torch.min(operator.illum)) / (torch.max(operator.illum)-torch.min(operator.illum)))+0.5
        # factor = w_factor * torch.mean(operator.illum)
        factor = torch.exp(torch.mean(operator.illum, dim=1))
        # print(w_factor)
        inv_denoised = invH1 + invG1

        # meas_r, meas_g, meas_b = torch.split(measurement, 1, dim=1)
        # meas_k1 = meas_g / meas_r
        # meas_k2 = meas_b / meas_r

        # meas_pred_r, meas_pred_g, meas_pred_b = torch.split(meas_pred, 1, dim=1)
        # meas_pred_k1 = meas_pred_g / meas_pred_r
        # meas_pred_k2 = meas_pred_b / meas_pred_r

        if w_meas > 0:  #alpha
            # use original calculate method
            # meas_pred = operator.forward(modify_origi_pred, is_latent=True, adaptive_beta=False)
            # compare = operator.illum * ((LG1+1.0)/2.0)
            # meas_error = self.loss_fn(measurement, meas_pred) + self.loss_fn(compare, meas_pred)
            # new_illu, r = operator.get_new_illum(origi_pred)
            # meas_R = operator.R_forward(origi_pred, new_illu, is_latent=True)
            # meas_L = operator.L_forward(origi_pred, r, is_latent=True)
            # meas_error = self.loss_fn(measurement, r*meas_L)
            # meas_error = self.loss_fn(measurement, meas_pred)
            # meas_error = self.loss_fn(factor*measurement, factor*meas_pred) + 10.0*(self.loss_color(meas_k1, meas_pred_k1) + self.loss_color(meas_k2, meas_pred_k2))
            
            meas_error = self.loss_fn(factor*measurement, factor*meas_pred) 

            # use Laplacian Pyramid
            # meas_pred_1 = operator.forward(IH1, operator.illum, is_latent=True, adaptive_beta=use_adpative_beta)
            # meas_pred_2 = operator.forward(IH2, iG2, is_latent=True)
            # meas_error = 0.5*self.loss_fn(LH1, meas_pred_1) + 0.5*self.loss_fn(LH2, meas_pred_2)
        ...
        if w_inv > 0:  #omega
            ...
            # inv_error = self.loss_fn(inv_denoised, origi_pred)
            # inv_H = operator.a * operator.illum * invH1
            # inv_error = self.loss_fn(meas_inv, origi_pred) + operator.b * self.loss_fn(IH1, inv_H)

            # use Laplacian Pyramid
            # inv_error = self.loss_fn(invG2, IG2)

            # w_inv=1.0
            # loss = loss + inv_error * w_inv
            if timestep.item() > 300:
                loss = loss + inv_error  * w_inv
            else:
                loss = loss + inv_error  * 2.0
        ...
        if w_inpaint > 0:  #gamma
            ...
            encoded_z_0 = self.vae.config.scaling_factor * vae.encode(
                ...
                # inv_denoised).latent_dist.mean
                # invG1).latent_dist.mean
                meas_inv).latent_dist.mean
            
            # pred_original_sample_G1 = self.vae.config.scaling_factor * vae.encode(
            #     IG1).latent_dist.mean
            # encoded_z_0_ = self.vae.config.scaling_factor * vae.encode(
            #     meas_inv).latent_dist.mean
            
            inpaint_error = self.loss_fn_latent(encoded_z_0, pred_original_sample)
            # inpaint_error = self.loss_fn_latent(encoded_z_0, pred_original_sample_G1)+ 0.5*self.loss_fn_latent(encoded_z_0_, pred_original_sample)
            # inpaint_error = self.loss_fn_latent(encoded_z_0_, pred_original_sample)
        ...
        if index % print_freq == 0:
            ...
            if w_inv > 0:
                ...
                img_save(inv_denoised, f"{name}_invdenoised_{timestep.item():04d}{ext}")
                # img_save(factor*invH1, f"{name}_invdenoisedH1_{timestep.item():04d}{ext}")
            ...
            # if w_meas > 0:
                ...
                # img_save(meas_pred_1, f"{name}_meas_pred1_{timestep.item():04d}{ext}")
                # img_save(meas_pred_2, f"{name}_meas_pred2_{timestep.item():04d}{ext}")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model_id",
        type=str,
        default='/home/eileen/Diffusion/DuduZhu/stable-diffusion-v1-5',
        help="model id",
    )
    parser.add_argument(
        "--step_size",
    ...
    output_list = [opt.test_id, opt.gamma, opt.omega, opt.alpha, opt.a, psnr, ssim, lpips]
    new_data = pd.DataFrame([output_list])
    new_data.to_csv("/home/eileen/Diffusion/DuduZhu/Diff_llie/psnr_ssim_lpips.csv", mode='a', header=False, index=False)
