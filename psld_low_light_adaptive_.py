...
from measurements_ import get_noise, DarkenOperator
...
class PLSDPipeline(DiffusionPipeline):
    ...
    def guidance_callback(
        ...
        if index % print_freq == 0:
            ...
            # img_save(LG1, f"{name}_LG1_{timestep.item():04d}{ext}")
            # img_save(IG2, f"{name}_IG2_{timestep.item():04d}{ext}")
            # img_save(LG2, f"{name}_LG2_{timestep.item():04d}{ext}")
            # img_save(IH1, f"{name}_IH1_{timestep.item():04d}{ext}")
            # img_save(LH1, f"{name}_LH1_{timestep.item():04d}{ext}")
            # img_save(IH2, f"{name}_IH3_{timestep.item():04d}{ext}")
            # img_save(LH2, f"{name}_LH2_{timestep.item():04d}{ext}")
...
if __name__ == "__main__":
    ...
    for i, imgName in enumerate(imgsName):
        ...
        operator = DarkenOperator(
            ...
            img_path=img_path, dtype=torch.float32, beta=opt.beta, sigma=opt.sigma
        )
