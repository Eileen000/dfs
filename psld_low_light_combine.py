...
class LaplacianPyramid(nn.Module):
    ...
    def combine_ag1_bh1(self, a, b):
        ...
        combined = torch.from_numpy(aG1+bH1).to('cuda').permute(2, 0, 1).unsqueeze(0) / 255.0 
...
class PLSDPipeline(DiffusionPipeline):
    ...
    def guidance_callback(
        ...
        if index % print_freq == 0:
            ...
            if w_meas > 0:
                img_save(meas_pred, f"{name}_meas_pred{timestep.item():04d}{ext}")            
    ...
    def __call__(
        ...
        L_img = LaplacianPyramid().combine_ag1_bh1(callback_kwargs['measurement'], init_image)
        name, ext = os.path.splitext(callback_kwargs['save_path'])
        img_save(L_img, f"{name}_combined_{ext}")
        latents = self.vae.config.scaling_factor * self.vae.encode(L_img).latent_dist.mean
